...
Để đảm bảo tính nhất quán giữa frontend và backend, các logic tính toán, định dạng và kiểm tra giá được tập trung trong file [pricing.ts](../../apps/web-1/lib/pricing.ts):
...
- **Endpoint cập nhật giá gói:** `PUT /api/admin/packages/:id/price`
  - **Controller Handler:** `updatePackagePriceHandler` trong [admin.controller.ts](../../apps/api/src/modules/admin/http/admin.controller.ts)
  - **Use Case:** [update-package-price.usecase.ts](../../apps/api/src/modules/admin/application/update-package-price.usecase.ts)
  - **Repository:** `updatePackagePrice` trong [package.repository.ts](../../apps/api/src/modules/packages/infrastructure/persistence/package.repository.ts)
  - **Đầu vào (Body Schema):**
    ```json
    {
      "price": number
...
- **API chi tiết Case:** `GET /api/cases/:id`
  - **Use Case:** [get-case-detail.usecase.ts](../../apps/api/src/modules/cases/application/get-case-detail.usecase.ts)
...
- **API Upload Payment Proof:** `POST /api/payments/upload-proof` (hoặc tương tự)
  - **Use Case:** [upload-payment-proof.usecase.ts](../../apps/api/src/modules/payments/application/upload-payment-proof.usecase.ts)
...
- **Custom Hook:** [useAdminPackages.ts](../../apps/web-1/app/admin/hooks/useAdminPackages.ts)
- **UI Admin Settings:** [AdminPackagesSettings.tsx](../../apps/web-1/app/admin/_components/AdminPackagesSettings.tsx) - hiển thị danh sách gói và lịch sử thay đổi giá cũ của Admin.
- **UI Student Workspace & Payment:**
  - [UnpaidAlertBanner.tsx](../../apps/web-1/app/dashboard/case/[id]/_components/UnpaidAlertBanner.tsx): Sử dụng `getCaseEffectivePrice` hiển thị số tiền cần trả trên banner thông báo.
  - [PaymentDrawer.tsx](../../apps/web-1/app/dashboard/case/[id]/_components/PaymentDrawer.tsx): Sử dụng `getCaseEffectivePrice` và `validatePaymentProof`.
  - [/payment/page.tsx](../../apps/web-1/app/dashboard/case/[id]/payment/page.tsx): Trang thanh toán chi tiết sử dụng `caseRequiresPayment`, `getCaseEffectivePrice` để tạo mã QR chuyển khoản chính xác và hiển thị thông tin hóa đơn.
