...
- **No Direct API Client Calls in UI Components**: Do not call `apiClient` directly with a URL inside UI components. Instead, extract the API calls into custom TanStack Query hooks (queries and mutations) and place them in the corresponding `hooks/` directory (e.g., `hooks/` within the same feature folder).
